...
def load_customer_dim(host,db_name,user,password):
    try:
        ...
        load_data_to_postgres('DIM_CUSTOMERS', customer_df,host,db_name,user,password)
...
def load_store_dim(host,db_name,user,password):
    try:
        ...
        load_data_to_postgres('DIM_STORE', store_df,host,db_name,user,password)
        logging.info('store_df dimension loaded successfully')
        store_df.to_csv('./datalake/gold/store_dim.csv', index=False)
    except Exception as E:
        logging.error(f'Error loading users dimension: {str(E)}')

def load_product_dim(host,db_name,user,password):
    try:
        logging.info('Loading customer dimension...')
        ...
        load_data_to_postgres('DIM_PRODUCT', product_df,host,db_name,user,password)
        logging.info('product_df dimension loaded successfully')
        product_df.to_csv('./datalake/gold/product_dim.csv', index=False)
    except Exception as E:
        logging.error(f'Error loading users dimension: {str(E)}')

def load_sales_fact(host,db_name,user,password):
    try:
